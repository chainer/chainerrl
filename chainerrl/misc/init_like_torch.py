...
def init_like_torch(link):
    ...
    for l in link.links():
        if isinstance(l, L.Linear):
            out_channels, in_channels = l.W.array.shape
            stdv = 1 / np.sqrt(in_channels)
            l.W.array[:] = np.random.uniform(-stdv, stdv, size=l.W.array.shape)
            if l.b is not None:
                l.b.array[:] = np.random.uniform(-stdv, stdv,
                                                size=l.b.array.shape)
        elif isinstance(l, L.Convolution2D):
            out_channels, in_channels, kh, kw = l.W.array.shape
